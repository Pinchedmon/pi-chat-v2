...
const SignupPage = () => {
    ...
    return (
        <section
            ...
            {isWelcome.current ? <>
                <SignupWelcomeForm onSwitch={() => isWelcome.current = false} />
            </> :
                <SignupFinishForm />
            }
            <div
                onClick={() => { isWelcome.current ? router.push('signin') : isWelcome.current = true }} className="mt-[45px] underline text-center text-[#37B34A] cursor-pointer">{isWelcome ? 'Выйти' : 'Назад'}</div>

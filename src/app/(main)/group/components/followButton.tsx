...
const FollowButton = () => {
    ...
    const { data, error } = useSWR(`/api/group/isSub?id=${id}&userId=${session.data?.user.id}`, fetcher);
    const subscribe = () => {
        axios.post('/api/group/subscribe', { groupId: id, userId: session.data?.user.id })
            .then(response => {
                mutate(`/api/group/isSub?id=${id}&userId=${session.data?.user.id}`)
            })
    }
    if (error) {
        return <div>Error loading subbutton!</div>;
    }
    if (!data || !id) {
        return <div>Loading subbutton...</div>;
    }
    return (
        <button className='flex mt-[10px] rounded-[20px] px-4 py-2 font-medium  bg-bg-content dark:bg-dark-bg-content' onClick={subscribe}>
            {data.isSub ?
                <>
                    <svg className="fill-black dark:fill-white mr-2" xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24">
                ...
                :
                <>
                    <svg className="fill-black dark:fill-white mr-2" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">

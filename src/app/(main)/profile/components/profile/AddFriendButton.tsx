...
const AddFriendButton = (props: { id: string, userId: string }) => {
    ...
    const { data, isLoading, error } = useSWR(`/api/friend?userId=${userId}&friendId=${id}`, fetcher)
    const addFriend = async () => {
        try {
            ...
            mutate(`/api/friend?userId=${userId}&friendId=${id}`)
        }
        catch (err) {
            console.log(err)
        }
    ...
    const deleteFriend = async () => {
        try {
            await axios.delete(`/api/friend?userId=${userId}&friendId=${id}`)
            mutate(`/api/friend?userId=${userId}&friendId=${id}`)
        }
        catch (err) {
            console.log(err)
        ...
        <>
            {
                ...
                <button onClick={() => data.friend ? deleteFriend() : addFriend()} className='ml-2 md:ml-0 flex mt-[10px] rounded-[20px] px-4  py-2 font-medium items-center bg-bg-content dark:bg-dark-bg-content'>
                    ...
                    <p className='text-[12px] md:text-[16px] ml-2'>{!data.friend ? "Добавить в друзья" : "Удалить из друзей"}</p>
                </button>
            }
        </>
    )
